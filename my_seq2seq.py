        ...
        for i, inp in enumerate(decoder_inputs):
            ...
            if i == 0:
                ...
                for kk in range(beam_size):
                    states.append(state)
                state = tf.reshape(tf.concat(states, axis=0), [-1, state_size])
...
def attention_decoder(decoder_inputs, initial_state, attention_states, cell,
    ...
    with variable_scope.variable_scope(scope or "attention_decoder"):
        ...
        outputs = []
        prev = None
        batch_attn_size = array_ops.stack([batch_size, attn_size], axis=0)  # array_ops.pack([batch_size, attn_size])
...
def embedding_attention_decoder(decoder_inputs, initial_state, attention_states,
    ...
    with variable_scope.variable_scope(scope or "embedding_attention_decoder"):
        with ops.device("/cpu:0"):
            embedding = variable_scope.get_variable("embedding",
                                                    [num_symbols, embedding_size])
        print("Check number of symbols: {}".format(num_symbols))
        if beam_search:
            loop_function = _extract_beam_search(
                embedding, beam_size, num_symbols, embedding_size, output_projection,
                update_embedding_for_previous)
